...
use super::tokenization::Token;

fn is_letter(ch: char) -> bool {
    ch.is_alphabetic() || ch == '_'
}

pub struct Lexer<'a> { input: Peekable<Chars<'a>>, }
...
impl<'a> Lexer<'a> {
    ...
    pub fn read_char(&mut self) -> Option<char> { self.input.next() }
    pub fn pick_char(&mut self) -> Option<&char> { self.input.peek() }
    ...
    pub fn next_token(&mut self) -> Token {
        ...
        match self.read_char() {
            ...
            Some(ch @ _) => {
                if is_letter(ch) {
                    let literal = self.read_id(ch);
                    tokenization::get_identifier(&literal)
